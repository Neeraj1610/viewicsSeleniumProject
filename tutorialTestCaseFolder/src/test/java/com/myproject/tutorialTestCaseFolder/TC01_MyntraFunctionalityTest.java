...
public class TC01_MyntraFunctionalityTest extends BaseTestSetup{

	@Test
	@ProjectCustomAnnotation(totalAppBrowserCount=1)
	public void testTC01_MyntraFunctionality() throws Exception
	{
		...
		//Login with driver1 instance
		 ...
		 try{
			 ...
			 softAssert.assertTrue(noOfItems.contains(bagCounter), "Wrong no of item in cart");
			 
			 //Verify right size is added in cart
			 String size = checkoutPagePajeObject.verifyRightSizeAddedInCart();
			 softAssert.assertTrue(size.contains("32"), "Wron size added to cart, size added is : "+ size);
			 
			 // Click on place order button
			 checkoutPagePajeObject.clickOnPlaceOrderButton();
