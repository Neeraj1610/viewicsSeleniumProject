...
public class SearchResultPageObject {
	...
	private void selectFilterAndValue(String filterValueToSelect) throws Exception{
		// Click on size filter label
	...
	public boolean isAddedToBagToastMsgPresent(){
		
		basePageObject.waitForElementVisibility(myntraRepository.addedToBagToastMsg);
		
		return basePageObject.isElementPresent(myntraRepository.addedToBagToastMsgXpath, "");
	}
	
	public String verifyBagCounterUpdated() throws Exception{
		basePageObject.waitForElementVisibility(myntraRepository.bagCounter);
		
		String counterText = basePageObject.getText(myntraRepository.bagCounterXpath);
		
		return counterText;
	}
	
	public void clickOnCheckOutCartIcon(){
		
		basePageObject.click(myntraRepository.bagIcon);
		
		// wait for page to load
		basePageObject.waitForPageToLoad();
		
		// Check checkout page loaded
		basePageObject.waitForElementVisibility(myntraRepository.myShoppingBag);
	}
}